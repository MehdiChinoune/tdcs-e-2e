program test_conhyp_01
  ...
  !$OMP PARALLEL DO COLLAPSE(6) PRIVATE(a, b, z, a_dp, b_dp, z_dp, zh ,ch)
  do ia = 1, 6
    do ja = 1, 6
      do ib = 1, 8
        do jb = 1, 8
          do iz = 1, 7
            do jz = 1, 7
              a = cmplx( i_tst(ia)*1._wp, i_tst(ja)*1._wp, wp)
              b = cmplx( i_tst(ib)*1._wp, i_tst(jb)*1._wp, wp)
              z = cmplx( i_tst(iz)*1._wp, i_tst(jz)*1._wp, wp)
              ch = conhyp(a, b, z, 0, 10)
              a_dp = cmplx( i_tst(ia)*1._dp, i_tst(ja)*1._dp, dp)
              b_dp = cmplx( i_tst(ib)*1._dp, i_tst(jb)*1._dp, dp)
              z_dp = cmplx( i_tst(iz)*1._dp, i_tst(jz)*1._dp, dp)
              zh = zhypgeom_1f1(a_dp, b_dp, z_dp)
              if( abs(ch-zh)/abs(zh)>1.e-7_wp ) then
                print*, "a= ", a
                print*, "b= ", b
                print*, "z= ", z
