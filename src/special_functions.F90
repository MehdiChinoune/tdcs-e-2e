...
CONTAINS
  ...
  CONTAINS
    ...
    ELEMENTAL REAL(RP) FUNCTION rexp(x)
      ...
      ! Local variables
      REAL(KIND=RP), PARAMETER  :: p1 = .914041914819518E-9_RP, p2 = .238082361044469E-1_RP, &
  ...
  PURE SUBROUTINE coul90(x, eta, lmin, lrange, fc, gc, fcp, gcp, kfn, ifail )
    ...
    REAL(KIND=RP), PARAMETER :: small = SQRT(TINY(1._RP)), ten2 = 100._RP, half = 0.5_RP, &
  ...
  ELEMENTAL SUBROUTINE  jwkb( x, eta, xl, fjwkb, gjwkb, iexp )
    ...
    REAL(KIND=RP), PARAMETER :: half = 0._RP, six = 6._RP, ten = 10._RP &
